"""
predecir-electricidad.py
--------------------------
Usa los dos modelos Electricidad entrenados simultáneamente:
  - Modelo 1: Nivel de riesgo     (BAJO / MEDIO / ALTO)
  - Modelo 2: Categorías          (multi-etiqueta, 5 dimensiones)

Uso:
  python predecir-electricidad.py
"""
...
# CONFIGURACIÓN
# ==========================================
RIESGO_MODEL_PATH     = "./modelo_electricidad_riesgo_final"
CATEGORIAS_MODEL_PATH = "./modelo_electricidad_categorias_final"
MAX_LENGTH            = 128
...
# ya que fue el tokenizer con el que se entrenó.
# ==========================================
def cargar_modelos():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Dispositivo: {device}\n")

    print("Cargando modelo de riesgo (Electricidad)...")
    tok_riesgo = ElectraTokenizerFast.from_pretrained(RIESGO_MODEL_PATH)
    mod_riesgo = AutoModelForSequenceClassification.from_pretrained(RIESGO_MODEL_PATH)
    mod_riesgo.to(device).eval()

    print("Cargando modelo de categorías (Electricidad)...")
    tok_cats = ElectraTokenizerFast.from_pretrained(CATEGORIAS_MODEL_PATH)
...
def main():
    print("=" * 65)
    print("   Predictor Electricidad — Riesgo + Categorías Pedagógicas")
    print("=" * 65)
